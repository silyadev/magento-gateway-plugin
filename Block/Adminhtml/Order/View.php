<?php
// Copy and edit ./vendor/magento/module-sales/Block/Adminhtml/Order/View.php

namespace Vendo\Gateway\Block\Adminhtml\Order;

use Magento\Sales\Block\Adminhtml\Order\View as MagentoSalesOrderView;
 ...
 * @since 100.0.2
 */
class View extends MagentoSalesOrderView
{
    /**
     * Block group
     *
    ...
    protected function _construct()
    {
        ...
        // *** Begin ***************************************************************************************************
        $vendoPaymentPreauthOnly = (bool)$this->paymentMethod->getConfigData('preauth_only');
        if ($order->getVendoPaymentResponseStatus() == 1 && $vendoPaymentPreauthOnly) {
            $message = __(
                'This will create an capture vendo step 2/2. Do you want to continue?'
            );

            $url = $this->storeManager->getStore()->getBaseUrl();
            if (!empty($url)) {
                $url .= 'vendo/preauthcapture/index/order_id/' . $order->getId();
                $onClick = "confirmSetLocation('{$message}', '{$url}')";
                $this->addButton(
                    'order_capture_vendo_step2',
                    ['label' => __('Capture (Vendo Step 2/2)'), 'onclick' => $onClick, 'class' => 'capture-vendo-step2']
                );
            }
        }
        // *** End *****************************************************************************************************
